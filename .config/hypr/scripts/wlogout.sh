...
if ((resolution >= 2160)); then
	...
	wlogout --css ~/.config/wlogout/vertical.css -l ~/.config/wlogout/layout --protocol layer-shell -b 6 -T $(awk "BEGIN {printf \"%.0f\", $A_1080 * 1080 * $hypr_scale / $resolution}") -B $(awk "BEGIN {printf \"%.0f\", $B_1080 * 1080 * $hypr_scale / $resolution}") &
